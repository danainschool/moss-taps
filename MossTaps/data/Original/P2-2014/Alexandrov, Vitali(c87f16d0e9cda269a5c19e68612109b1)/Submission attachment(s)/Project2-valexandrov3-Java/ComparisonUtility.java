...
public class ComparisonUtility {
    ...
    // Get the number of objects in the figure
    public int numObjects(RavensFigure figure) {
        int num = 0;
        for (String objectName : figure.getObjects().keySet()) {
            num++;
        }
    ...
    // Find the number of objects of each type in the figure and store it as a HashMap
    public HashMap<String, Integer> objectsQuantity (RavensFigure figure) {
        ...
        for (String objectName : figure.getObjects().keySet()) {
            ...
            for (String attributeName : thisObject.getAttributes().keySet()) {
                ...
                if (attributeName.equals("shape") && !map.containsKey(attributeValue)) {
                    map.put(attributeValue, num++);
                } else if (attributeName.equals("shape") && map.containsKey(attributeValue)) {
                    map.put(attributeValue, num++);
                }
    ...
    // Calculate the number difference for the same attribute name
    public HashMap<String, Integer> objectsDifference (HashMap<String, Integer> first, HashMap<String, Integer> second) {

        HashMap<String, Integer> map = new HashMap<String, Integer>();
        for (String keyInFirst : first.keySet()) {
            for (String keyInSecond : second.keySet()) {
                if (keyInFirst.equals(keyInSecond)) {
                    int diff = first.get(keyInFirst) - second.get(keyInSecond);
                    map.put(keyInFirst, diff);
                }
