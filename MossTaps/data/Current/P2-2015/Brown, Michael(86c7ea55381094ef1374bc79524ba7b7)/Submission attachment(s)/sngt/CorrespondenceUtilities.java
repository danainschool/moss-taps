...
public final class CorrespondenceUtilities 
{
	...
	{
		// Object similarity is based on Nesting level, Shape, Size, Fill, Angle (if available).  
		...
		if(from.getAttributes().get("angle") != null && to.getAttributes().get("angle") != null)
			if(from.getAttributes().get("angle").equals(to.getAttributes().get("angle")))
				score += 1;
		
		// Additional points granted to objects that are identical ignoring nesting
		if(TransformationUtilities.ObjectsIdentical(from, to, objectNesting, true))
			score += 5;
