...
public class Agent {
    ...
    public int Solve(RavensProblem problem) {
        ...
        try {
            if ( "2x2".equals(problem.getProblemType())){
                return solve22RPM(problem);
            }else{
                return solve33RPM(problem);
            }
    ...
    private Integer solve22RPM(final RavensProblem problem){
        buildABEdgeProperties(problem);
        final Integer answer = buildCDEdgeProperties(problem);
        return answer;
    }

    private Integer solve33RPM(final RavensProblem problem){
        //start building 3 X 3 problem sets between A, B and C and then D, E and F
        //then build between G, H and each of the answer.
        EdgeObjectProperties edgeObjectABProperties = buildEdgeProperties("A", "B", problem);
        EdgeObjectProperties edgeObjectBCProperties = buildEdgeProperties("B", "C", problem);
        EdgeObjectProperties edgeObjectDEProperties = buildEdgeProperties("D", "E", problem);
        EdgeObjectProperties edgeObjectEFProperties = buildEdgeProperties("E", "F", problem);
        EdgeObjectProperties edgeObjectGHProperties = buildEdgeProperties("G", "H", problem);

        //NOW ABC PROPERTIES MAP by iterating two EdgeObjectProperties
        final HashMap<String, String> combneHashMapAB = combineProperties(edgeObjectABProperties.getEdgeObjectChangeProperties().get("A1").getObjectChangeProperties(),
                edgeObjectBCProperties.getEdgeObjectChangeProperties().get("A1").getObjectChangeProperties());
        final HashMap<String, String> combneHashMapDE = combineProperties(edgeObjectABProperties.getEdgeObjectChangeProperties().get("A1").getObjectChangeProperties(),
                edgeObjectBCProperties.getEdgeObjectChangeProperties().get("A1").getObjectChangeProperties());
        final Integer answer = findAnswer(problem);
        return answer;
    }

    private HashMap<String, String> combineProperties(HashMap<String, String> oneTwo, HashMap<String, String> twoThree){

        final HashMap<String, String> combineHashMap = new HashMap<>();

        for ( final String oneTwoKey: oneTwo.keySet()){

            for ( final String twoThreeKey: twoThree.keySet()){
                if ( oneTwo.get(oneTwoKey).equals(twoThreeKey)){
                    combineHashMap.put(new StringBuilder(oneTwoKey).append("-").append(twoThreeKey).toString(),
                            new StringBuilder(oneTwoKey).append("-").append(twoThree.get(twoThreeKey)).toString());
                }
            }
        }
        return combineHashMap;
    }

    private EdgeObjectProperties buildEdgeProperties(final String image1Key, final String image2Key, final RavensProblem problem){
        final HashMap<String, RavensFigure> ravenFigures = problem.getFigures();
        final RavensFigure ravensFigureA = ravenFigures.get(image1Key);
        final RavensFigure ravensFigureB = ravenFigures.get(image2Key);
    ...
    private void buildABCEdgeProperties(final RavensProblem problem){
        final HashMap<String, RavensFigure> ravenFigures = problem.getFigures();
        final RavensFigure ravensFigureA = ravenFigures.get("A");
        final RavensFigure ravensFigureB = ravenFigures.get("B");
    ...
    private Integer findAnswer(final RavensProblem problem){
        final HashMap<String, EdgeObjectProperties> answerEdgeObjectPropertiesHashMap = new HashMap<>();
        return 3;
    }
