...
/**
 * This class is used to contain Transitions between figures
 */
public class RavensTransition {
            ...
            if(currentLarger == null) {
                ...
                correspondingObjects.forEach((k,v) -> {
                    if(v == null){
                        ...
                        t.add(new Transition(TransitionType.ADDED, this.figureNet.getFigure()
                                .getObjects()
                                .get(k)
                                .getAttributes()
                                .get("shape")));
                ...
                correspondingObjects.forEach((k,v) -> {
                    if(v == null){
                        List<Transition> t = new ArrayList<>();
                        t.add(new Transition(TransitionType.DELETED, fromFigure.getFigure()
                                .getObjects()
                                .get(k)
                                .getAttributes()
                                .get("shape")));
    ...
    private HashMap<String, RavensObject> getCorrespondingObjects(FigureNetwork fromFigure){
        ...
        if(currentLarger != null && currentLarger && fromFigure.getObjectCount() == 0){
            fromFigure.getObjects().forEach(v -> corresponding.put(v.getName() ,null));
            return corresponding;
        }
        else if(this.figureNet.getObjectCount() == 0){
            this.figureNet.getObjects().forEach(v -> corresponding.put(v.getName(), null));
            return  corresponding;
        }
    ...
    private static List<Transition> getTransitions(RavensObject initialObj, RavensObject currentObj) {
        ...
        if(hasExpandedWidth(initialObj, currentObj)){
            transitions.add(TransitionFactory.getTransition(TransitionType.EXPANDED_W, initialObj, currentObj));
        }

        if(hasExpandedHeight(initialObj, currentObj)){
            transitions.add(TransitionFactory.getTransition(TransitionType.EXPANDED_H, initialObj, currentObj));
        }
    ...
    private static boolean hasExpandedHeight(RavensObject initial, RavensObject current){
        HashMap<String, String> initialAttr = initial.getAttributes(), currentAttr = current.getAttributes();

        if(initialAttr.containsKey("height") && currentAttr.containsKey("height")){
            return !initialAttr.get("height").equalsIgnoreCase(currentAttr.get("height"));
        }
        else if((initialAttr.containsKey("height") && !currentAttr.containsKey("height")) ||
                !initialAttr.containsKey("height") && currentAttr.containsKey("height"))
            return true;
        else
            return false;
    }

    private static boolean hasExpandedWidth(RavensObject initial, RavensObject current){
        HashMap<String, String> initialAttr = initial.getAttributes(), currentAttr = current.getAttributes();

        if(initialAttr.containsKey("width") && currentAttr.containsKey("width")){
            return !initialAttr.get("width").equalsIgnoreCase(currentAttr.get("width"));
        }
        else if((initialAttr.containsKey("width") && !currentAttr.containsKey("width")) ||
                !initialAttr.containsKey("width") && currentAttr.containsKey("width"))
            return true;
        else
            return false;
    }
