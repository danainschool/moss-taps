...
def test(networks, transformer):
    absig = json.dumps(transformer(networks['a']['b']))
    c1sig = json.dumps(transformer(networks['c']['1']))
    c2sig = json.dumps(transformer(networks['c']['2']))
    c3sig = json.dumps(transformer(networks['c']['3']))
    c4sig = json.dumps(transformer(networks['c']['4']))
    c5sig = json.dumps(transformer(networks['c']['5']))
    c6sig = json.dumps(transformer(networks['c']['6']))

    acsig = json.dumps(transformer(networks['a']['c']))
    b1sig = json.dumps(transformer(networks['b']['1']))
    b2sig = json.dumps(transformer(networks['b']['2']))
    b3sig = json.dumps(transformer(networks['b']['3']))
    b4sig = json.dumps(transformer(networks['b']['4']))
    b5sig = json.dumps(transformer(networks['b']['5']))
    b6sig = json.dumps(transformer(networks['b']['6']))

    bcsig = json.dumps(transformer(networks['b']['c']))
    a1sig = json.dumps(transformer(networks['a']['1']))
    a2sig = json.dumps(transformer(networks['a']['2']))
    a3sig = json.dumps(transformer(networks['a']['3']))
    a4sig = json.dumps(transformer(networks['a']['4']))
    a5sig = json.dumps(transformer(networks['a']['5']))
    a6sig = json.dumps(transformer(networks['a']['6']))
